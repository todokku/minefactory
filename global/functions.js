...
module.exports = (bot, utils, ytdl, config) => {
    ...
    fs.readdir("./commands/", (err, files) => {
        ...
        if (jsfiles.length <= 0) return console.log("Parancs betöltése...");

        console.log(`Loading ${jsfiles.length} Parancsok....`);
        jsfiles.forEach((f, i) => {
    ...
    bot.handleVideo = async (video, message, vc, playlist = false) => {
        ...
        if (!queue) {
            ...
            try {
                ...
                console.error(`Nem vagy csatlakozva duma szobához: ${err}`);
            }
        } else {
            queue.musics.push(music);
            if (playlist) return;
            else return message.channel.send(`Lejátszási lista: **${music.title}** Hozzáadva a várólistához!`);
        }
        return;
    }
    ...
    bot.play = (guild, music) => {
        ...
        queue.textChannel.send(`Most megy: **${music.title}**`);
    }
